...
public class RoleServiceImpl implements RoleService {
    ...
    @Override
    public ResponseVo<TRole> detail(Long roleId) {
        TRole tRole = roleMapper.selectByPrimaryKey(roleId);
        return ResponseVo.success(tRole);
    }

    @Override
    public ResponseVo<Boolean> updOrSave(TRole role) {
        if (role.getId() == null) {
    ...
    @Override
    public ResponseVo<Boolean> changeStatus(Long roleId, Byte sta) {
        TRoleExample roleExample = new TRoleExample();
        roleExample.createCriteria()
                .andStatusEqualTo(HuamiaoConst.ZERO)
                .andIdEqualTo(roleId);
    ...
    @Override
    public PageVo<TRole> queryList(BaseParam baseParam) {
